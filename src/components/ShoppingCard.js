      ...
      <div className="w-full p-4">
        <div className="flex items-center justify-between mb-3">
          ...
          <button onClick={(e)=> removeFromCartHandler(item)}>
            <TrashIcon className="w-5 h-5 md:w-6 md:h-6 hover:text-primary transition duration-200" />
          </button>
        </div>
          ...
          <div>
            <div className="bg-tint-100 text-primary flex items-center gap-1.5 rounded p-1">
              <button onClick={(e)=> increaseQuantityHandler(item)}>
                <PlusSmallIcon className="w-4 h-4 md:w-5 md:h-5" />
              </button>
              <span className="text-sm md:text-base w-4 text-center">{item.quantity}</span>
              <button onClick={(e)=> decreaseQuantityHandler(item)}>
                {item.quantity > 1 && (<MinusSmallIcon className="w-4 h-4 md:w-5 md:h-5" />)}
                {item.quantity === 1 && (<TrashIcon className="w-4 h-4 md:w-5 md:h-5" />)}
              </button>
            </div>
          </div>
          <div>
